...
# Sends sensor data and receives manual control command
# ============================================================
...
def read_dht_safe():
    # Safely read DHT sensor, handling potential errors
    try:
        t = dht.temperature
        h = dht.humidity
...
def send_sensor_data(temp, hum, soil_pct, irrigation_status, ai_reason):
    ...
    try:
        requests.post(SENSOR_DATA_ENDPOINT, json=payload, timeout=5)
    except requests.exceptions.RequestException as e:
        print(f"[{datetime.now().strftime('%H:%M:%S')}] ✗ Network Error sending data.")

def get_manual_control_status():
    """Fetch manual control status from the server."""
    try:
        response = requests.get(CONTROL_STATUS_ENDPOINT, timeout=3)
        if response.status_code == 200:
            data = response.json()
            return data.get('manual_enabled', False), data.get('pump_command', False)
        return False, False
    except requests.exceptions.RequestException:
        return False, False # Revert to safe automated mode on connection failure
...
with open(logfile, "w", newline="") as f:
    ...
    try:
        while True:
            ...
            # [Logic for moving averages and delta]
            ...
            if manual_enabled:
                # If manual control is enabled, override AI
                final_decision = pump_command_manual
                reason = "MANUAL"
            else:
                # If automated control, follow AI and emergency rules
            ...
            # Turn OFF after duration (to ensure safe operation or stopping after a short manual request)
            if pump_on and (now - last_change) >= MIN_ON_SEC and ((now - on_start) >= BURST_ON_SEC):
                # If in manual mode and manual command is OFF, stop immediately
                if manual_enabled and not pump_command_manual:
                     is_time_to_stop = True
                else:
                    # In automatic mode, stop after BURST_ON_SEC
                    is_time_to_stop = (now - on_start) >= BURST_ON_SEC
                    
                if is_time_to_stop or (now - on_start) >= MAX_ON_SEC:
                    relay_set(False)
                    pump_on=False
                    last_change=now
